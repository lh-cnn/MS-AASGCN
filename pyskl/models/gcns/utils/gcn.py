...
class unit_aagcn(nn.Module):
    def __init__(self, in_channels, out_channels, A, coff_embedding=4, adaptive=True, attention=False):
        ...
        self.cbam = EfficientChannelAttention(out_channels)
        ...
        if self.adaptive:
            ...
            self.register_buffer('A', A)
            self.AD = Variable(A, requires_grad=False)
            self.BD = nn.Parameter(A)
